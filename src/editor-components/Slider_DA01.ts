...
function renderLoop(imgs: string[], n: number, iv: number, w: number, h: number): string {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const sl = ((i + 1) * iv) / dur
    if (i === 0) { pts.push(0); vals.push('0 0') }
    pts.push(+se.toFixed(4)); vals.push(`${-i * w} 0`)
    pts.push(+sl.toFixed(4)); vals.push(`${-(i + 1) * w} 0`)
  }
  const cse = (dur - st) / dur
  pts.push(+cse.toFixed(4)); vals.push(`${-n * w} 0`)
  pts.push(0.999);          vals.push(`${-n * w} 0`)
  pts.push(1);              vals.push('0 0')
...
function renderBounce(imgs: string[], n: number, iv: number, w: number, h: number): string {
  ...
  // 正向滚动 n-1 次
  for (let i = 0; i < n - 1; i++) {
    t += iv - st;  pts.push(+(t / dur).toFixed(4)); vals.push(`${-i * w} 0`)
    t += st;       pts.push(+(t / dur).toFixed(4)); vals.push(`${-(i + 1) * w} 0`)
  }

  // 末端停留
  t += iv
  pts.push(+(t / dur).toFixed(4)); vals.push(`${-(n - 1) * w} 0`)

  // 反向滚动 n-1 次
  for (let i = n - 2; i >= 0; i--) {
    t += st;       pts.push(+(t / dur).toFixed(4)); vals.push(`${-i * w} 0`)
    t += iv - st;  pts.push(+(t / dur).toFixed(4)); vals.push(`${-i * w} 0`)
  }

  if (pts[pts.length - 1] < 0.999) { pts.push(1); vals.push('0 0') }
...
function renderJumpBack(imgs: string[], n: number, iv: number, w: number, h: number): string {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    const sl = ((i + 1) * iv) / dur
    if (i === 0) {
      pts.push(+se.toFixed(4)); vals.push('0 0')
    } else {
      pts.push(+se.toFixed(4)); vals.push(`${-i * w} 0`)
    }
    pts.push(+sl.toFixed(4)); vals.push(`${-(i + 1) * w} 0`)
  ...
  // 最后一张停留
  pts.push(+((n * iv) / dur).toFixed(4)); vals.push(`${-(n - 1) * w} 0`)
...
export const Slider_DA01 = {
  ...
  attrs: [
    ...
    { key: 'type',     label: '轮播类型', required: false, default: '1', options: ['1(循环)', '2(来回)', '3(滚回)', '4(淡入淡出)'] },
  ],
  ...
  render(attrs: Record<string, string>, _body: string, _t: ThemeColors): string {
    ...
    switch (type) {
      case 2: return renderBounce(images, count, interval, width, height)
      case 3: return renderJumpBack(images, count, interval, width, height)
      case 4: return renderFade(images, count, interval, width, height)
      default: return renderLoop(images, count, interval, width, height)
    }
  },
}
