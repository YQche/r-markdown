  ...
  attrs: [
    {
      ...
      required: true,
      default: '',
      description: '图片 URL 列表，多个地址用英文逗号分隔',
    },
    {
      key: 'interval',
      label: '每张显示时长（秒），最小2秒',
      required: false,
      default: '3',
      description: '每张图片展示的时长（秒），最低 2 秒',
    },
    {
      key: 'width',
      label: '视图宽度，可根据使用图片调整',
      required: false,
      default: '600',
      description: '视图宽度（像素），根据图片实际尺寸调整',
    },
    {
      key: 'height',
      label: '视图高度，可根据使用图片调整',
      required: false,
      default: '200',
      description: '视图高度（像素），根据图片实际尺寸调整',
    },
    {
      key: 'type',
      label: '轮播类型',
      ...
      description: '轮播类型：1（循环滚动）/ 2（来回滚动）/ 3（滚回）/ 4（淡入淡出）',
    },
  ],
