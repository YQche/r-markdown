...
import { renderFrontMatter, parseSteps, parseBadges, parseCtaBlock, parseBreaking, parseCtaInline, parseCompare, parseCallout, parseEngage, parseGallery, } from './components';
export function parseMarkdown(md, t) {
    ...
    while (i < lines.length) {
        ...
        if (/^-\s*\[案例\s*\d+\]/.test(line)) {
            ...
            while (i < lines.length && /^-\s*\[案例\s*\d+\]/.test(lines[i])) {
                ...
                if (cm)
                    cases.push({ badge: cm[1], text: cm[2] });
                i++;
            }
