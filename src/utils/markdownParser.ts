      ...
      while (i < lines.length && /^[-*+]\s/.test(lines[i])) {
        ...
        if (cb) {
          ...
          const checkSvg = isChecked
            ...
            : `<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><rect x="1" y="1" width="10" height="10" rx="2" stroke="${uncheckedBorder}" stroke-width="1.5" fill="none"/></svg>`
          html += `<section style="margin:5px 0px"><span style="display:inline-flex;align-items:center;gap:8px"><span style="width:18px;height:18px;border-radius:4px;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;${isChecked ? `background:${t.accent};border-color:${t.accent}` : `border:2px solid ${uncheckedBorder}`}">${checkSvg}</span><span>${inlineFormat(cb[2], t)}</span></span></section>`
        } else {
